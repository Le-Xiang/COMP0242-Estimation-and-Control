...
class RegulatorModel:
    ...
    def updateSystemMatrices(self,sim,cur_x,cur_u):
        ...
        ##### option 1
        class RegulatorModelFixedLinearization:
            def __init__(self, N, q, m, n, dt):
                self.A = None
                self.B = None
                self.C = np.eye(q)  # 输出矩阵 C 为单位矩阵
                self.N = N
                self.q = q  # 输出维度
                self.m = m  # 控制输入维度
                self.n = n  # 状态维度
                self.dt = dt  # 离散时间步长
                self.linearize_system([0, 0, 0], [0, 0])  # 固定的线性化点

            def linearize_system(self, cur_x, cur_u):
                theta = cur_x[2]  # 当前的角度
                v = cur_u[0]      # 当前的线速度
                ...
                # 离散化矩阵 A 和 B
                self.A = np.eye(self.n) + self.dt * A_c
                self.B = self.dt * B_c
        ...
        n = 3  # 状态维度
        dt = 0.1
        model_fixed = RegulatorModelFixedLinearization(N, q, m, n, dt)
        print("Fixed A matrix:\n", model_fixed.A)
        print("Fixed B matrix:\n", model_fixed.B)
        

        class RegulatorModelDynamicLinearization:
            def __init__(self, N, q, m, n, dt):
                self.A = None
                self.B = None
                self.C = np.eye(q)  # 输出矩阵 C 为单位矩阵
            ...
            def updateSystemMatrices(self, cur_x, cur_u):
                theta = cur_x[2]  # 当前的角度
                v = cur_u[0]      # 当前的线速度
        ...
        # 使用示例
        model_dynamic = RegulatorModelDynamicLinearization(N, q, m, n, dt)
        current_state = [1.0, 0.5, np.pi / 4]  # 示例状态
        current_control = [1.0, 0.1]  # 示例控制输入
        model_dynamic.updateSystemMatrices(current_state, current_control)
        print("Dynamic A matrix:\n", model_dynamic.A)
        print("Dynamic B matrix:\n", model_dynamic.B)
    ...
    # TODO you can change this function to allow for more passing a vector of gains 被我tab往后一格了
