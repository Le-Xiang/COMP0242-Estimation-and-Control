...
from regulator_model_t4 import RegulatorModel
import json
from robot_localization_system_t4 import RobotEstimator, FilterConfiguration, Map

from test_func_t4 import Dataset
import pandas as pd
...
col = ["init_x_idx", "loop_num", "base_pos_x", \
       "base_pos_y", "base_bearing", "x_true_x", \
        "x_true_y", "x_true_theta", "x_est_x", \
        "x_est_y", "x_est_theta", "Sigma_est_x", \
        "Sigma_est_y", "Sigma_est_theta"]
...
def bearing2quaternion(bearing):
    # Convert bearing to quaternion
    quat = pin.Quaternion(np.cos(bearing/2), 0, 0, np.sin(bearing/2))
    
    return [round(i, 4) for i in list(quat.coeffs())]


# read config from the json file.
def get_config_path(conf_file_name):
...
def read_init_config(conf_file_name):
    conf_file_path = get_config_path(conf_file_name)
...
def set_init_position_config(conf_file_name, x0):
    config = read_init_config(conf_file_name)
    x = [float(x) for x in x0[0:2]] + [0.0]
    config.get("robot_pybullet").get("init_link_base_position")[0] = x
    config.get("robot_pybullet").get("init_link_base_orientation")[0] = list(bearing2quaternion(x0[2]))
    print(f"config.get('robot_pybullet').get('init_link_base_position')[0]:{config.get('robot_pybullet').get('init_link_base_position')[0]}")
    print(f"config.get('robot_pybullet').get('init_link_base_orientation')[0]:{config.get('robot_pybullet').get('init_link_base_orientation')[0]}")
...
def init_ekf(conf_file_name, x0):
    ...
    filter_config.x0 = np.array([x0[0], x0[1], x0[2]])

    print(f"filter_config.x0:{filter_config.x0}")  
...
def process_experiment(init_x_idx, x0, loop_num):
    # Configuration for the simulation
    conf_file_name = "robotnik_t4.json"  # Configuration file for the robot
    ...
    x_true_theta = [x[2] for x in x_true_history]
    x_est_x = [x[0] for x in x_est_history]
    x_est_y = [x[1] for x in x_est_history]
    x_est_theta = [x[2] for x in x_est_history]
    Sigma_est_x = [x[0] for x in Sigma_est_history] 
    Sigma_est_y = [x[1] for x in Sigma_est_history]
    Sigma_est_theta = [x[2] for x in Sigma_est_history]

    new_data = pd.DataFrame({
        "init_x_idx": idx_list,
        "loop_num": loop_num_list,
        "base_pos_x": base_pos_x,
        "base_pos_y": base_pos_y,
        "base_bearing": base_bearing_all,
        "x_true_x": x_true_x,
        "x_true_y": x_true_y,
        "x_true_theta": x_true_theta,
        "x_est_x": x_est_x,
        "x_est_y": x_est_y,
        "x_est_theta": x_est_theta,
        "Sigma_est_x": Sigma_est_x,
        "Sigma_est_y": Sigma_est_y,
        "Sigma_est_theta": Sigma_est_theta
    })
...
def main():
    ...
    for experiment_type in experiment_type_list:
        ...
        for j in range(len(initial_states)):
            ...
            # set the initial position in the config file
            set_init_position_config("robotnik_t4.json", x0)
            ...
            for i in range(dataset.num_experiments):
                ...
                df_cur = process_experiment(j, x0, i)
                if df.empty:
                    df = df_cur
                else:
            ...
            output_dir = os.path.join(current_directory, "results", experiment_type)
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            file_path_name = os.path.join(output_dir, f"experiment_position_idx_{j}.csv")
            # print(f"Save the data to {file_path_name}")
