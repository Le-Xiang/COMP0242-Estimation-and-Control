...
class Simulator(object):
    ...
    # Predict the state forwards to the next timestep, v is noise here
    def step(self):
        dt = self._config.dt
        v_c = self._u[0]
        omega_c = self._u[1]
    ...
    # Get the observations to the landmarks. Return None if none visible, W is the sensor detetion noise
    ...
    def landmark_range_bearing_observations(self):
        y = []
        C = []
        W = self._filter_config.W_bearing  # Use bearing noise covariance
        for lm in self._map.landmarks:
            # True range measurement (with noise)
            dx = lm[0] - self._x_true[0]
            dy = lm[1] - self._x_true[1]
            range_true = np.sqrt(dx**2 + dy**2)
            range_meas = range_true + np.random.normal(0, np.sqrt(self._filter_config.W_range))
            ...
            bearing_meas = wrap_angle(bearing_meas)  # Ensure the angle is wrapped

            # Append both range and bearing measurements
...
# Plot the 2 standard deviation and error history for each state. the sigma here is the confindence interval
state_name = ['x', 'y', 'θ']
