...
class RobotEstimator(object):
    ...
    def update_from_landmark_range_bearing_observations(self, y_range_bearing):
        ...
        # Separate range and bearing observations
        y_range = y_range_bearing[:, 0]
        y_bearing = y_range_bearing[:, 1]
        ...
        # Innovation. Look new information! (geddit?)
        nu_range = y_range - y_pred[:len(y_range)]
        nu_bearing = wrap_angle(y_bearing - y_pred[len(y_range):])  # Ensure angle wrapping
        nu = np.concatenate((nu_range, nu_bearing))
        print(nu.shape)

        # Build the covariance matrix
        W_landmarks = np.diag([self._config.W_range] * len(y_range) + [self._config.W_bearing] * len(y_bearing))
        self._do_kf_update(nu, C, W_landmarks)
