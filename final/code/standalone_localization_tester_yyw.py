...
from robot_localization_system_yyw import FilterConfiguration, Map, RobotEstimator
...
#Simulator 类模拟了机器人在二维平面中的运动。
#每次调用 step() 时，机器人根据当前的控制输入（线速度和角速度）更新位置。这里还加入了过程噪声 v，以模拟真实世界中的不确定性。
class Simulator(object):
    ...
    # 该函数计算机器人到地标的距离测量值。真实距离通过机器人和地标的相对位置计算得出，并且加入了测量噪声
...
# Main loop
#解释：
#每个时间步，控制输入被发送到仿真器，仿真器更新机器人的真实状态。
#扩展卡尔曼滤波器使用相同的控制输入预测机器人的下一步状态。
#地标测量被用于更新EKF的估计值，更新后的状态估计再次被用于计算下一步的控制输入。
#仿真中所有状态和估计值被存储下来以便后续绘图。
